...
const { sendDecisionEmail } = require('./notificationService');
...
async function createRequestFromEmail(emailData) {
  ...
  const senderDisplayName = getSenderDisplayName({ sender_name: fromName, sender_email: fromEmail });
...
async function updateRequestStatus({ id, status, note, actorUserId }) {
  const current = await query(`
    SELECT r.id, r.status, r.sender_name, r.sender_email, r.subject,
           r.request_type_code, rt.name AS request_type_name
    FROM requests r
    LEFT JOIN request_types rt ON rt.id = r.request_type_id
    WHERE r.id = $1
    LIMIT 1
  ...
  try {
    const notification = await sendDecisionEmail({
    ...
    if (notification.skipped) {
      await query(
        `INSERT INTO request_events (request_id, action, actor_user_id, note, meta)
         VALUES ($1, 'NOTIFICATION_SKIPPED', $2, $3, $4)`,
        [id, actorUserId, notification.reason, { notification }]
      );
      return { notification };
    }
    ...
    await query(
      ...
      [id, actorUserId, `Decision email sent to ${current.rows[0].sender_email || 'recipient'}`, { notification }]
    );
    ...
    await query(
      `INSERT INTO request_events (request_id, action, actor_user_id, note, meta)
       VALUES ($1, 'NOTIFICATION_FAILED', $2, $3, $4)`,
      [id, actorUserId, error.message, { details: error.details || null }]
    );
    return { notification: { ok: false, error: error.message, details: error.details || null } };
  }
}
