...
BREVO_SENDER_NAME=Crew Request System
BREVO_SENDER_EMAIL=no-reply@yourdomain.com
BREVO_REPLY_TO_NAME=HR Team
BREVO_REPLY_TO_EMAIL=hr@yourdomain.com
TZ=Asia/Baku
```
...
- Email notification approve/reject cavabı (Brevo API ilə)
...
## Approve / Reject zamanı avtomatik email

Bu layihədə approve və reject qərarı veriləndə request göndərən şəxsə avtomatik notification email göndərmək mümkündür.

Tövsiyə olunan yol: **Brevo Transactional Email API**.

Niyə?
- Railway free plan-da SMTP istifadə etmək məhduddur
- Brevo HTTP API Railway üzərində problemsiz işləyir
- Approve / reject düyməsi sıxılan anda birbaşa email gedə bilər

### Lazım olan env dəyişənləri
...
### İş prinsipi
- Request `APPROVED` olanda sender-ə approved email gedir
- Request `REJECTED` olanda sender-ə rejected email gedir
- Approver note yazsa, email içinə əlavə olunur
- Email göndərilməsi alınmasa belə request status update olunur, hadisə audit log-da `NOTIFICATION_FAILED` kimi saxlanır

### Vacib qeyd
`BREVO_SENDER_EMAIL` Brevo-da doğrulanmış sender/domain olmalıdır. Əks halda Brevo email-i reject edə bilər.
